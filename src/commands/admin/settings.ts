...
import { t, supportedLanguages, isValidLanguage } from "../../utils/i18n";
...
export const command: Command = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((s) =>
      s.setName("view").setDescription("Lihat pengaturan saat ini / View current settings")
    )
    .addSubcommand((s) =>
      s.setName("language").setDescription("Ubah bahasa bot / Change bot language")
        .addStringOption((o) =>
          o.setName("lang").setDescription("Bahasa / Language").setRequired(true)
            .addChoices(
    ...
    .addSubcommand((s) =>
      s.setName("djrole").setDescription("Atur peran DJ / Set DJ role")
        .addRoleOption((o) => o.setName("role").setDescription("Peran DJ (kosongkan untuk hapus)"))
    )
    .addSubcommand((s) =>
      s.setName("volume").setDescription("Atur volume default / Set default volume")
        .addIntegerOption((o) =>
          o.setName("level").setDescription("Volume (1-100)").setRequired(true)
            .setMinValue(1).setMaxValue(100)
        )
    )
    .addSubcommand((s) =>
      s.setName("announce").setDescription("Toggle pengumuman lagu / Toggle song announcements")
        .addBooleanOption((o) => o.setName("enabled").setDescription("Aktifkan?").setRequired(true))
    )
    .addSubcommand((s) =>
      s.setName("maxqueue").setDescription("Atur batas antrian / Set max queue size")
        .addIntegerOption((o) =>
          o.setName("size").setDescription("Ukuran maksimal (10-500)").setRequired(true)
            .setMinValue(10).setMaxValue(500)
        )
  ...
  async execute(interaction: ChatInputCommandInteraction, client: BotClient) {
    ...
    if (sub === "view") {
      const djRole = settings.djRoleId
        ? interaction.guild?.roles.cache.get(settings.djRoleId)?.name || "Unknown"
        : "Tidak ada / None";

      const embed = new EmbedBuilder()
        .setColor(0x5865f2)
        .setTitle(t("settings.title", lang))
        .addFields(
    ...
    if (sub === "language") {
      ...
      if (!isValidLanguage(langCode)) {
        return void interaction.reply({ embeds: [createErrorEmbed("Bahasa tidak valid!")], ephemeral: true });
      }
      ...
      await interaction.reply({
        embeds: [
          createSuccessEmbed(
            role
              ? t("settings.djRole", lang, { role: role.name })
              : "DJ role dihapus / DJ role removed"
          ),
        ],
      });
      ...
      const status = enabled ? t("status.on", lang) : t("status.off", lang);
      await interaction.reply({ embeds: [createSuccessEmbed(t("settings.announce", lang, { status }))] });
